      ...
      <vs-dropdown-menu class="cart-dropdown vx-navbar-dropdown" :class="{'dropdown-custom': cartItems.length}">
          ...
          <template v-if="cartItems.length">
              ...
              <ul class="bordered-items">
                  <li
                    ...
                    @click="$router.push({name: 'ecommerce-item-detail-view', params: {item_id: item.id }}).catch(() => {})">
                      ...
                      <!-- IMG COL -->
                      <div class="vx-col w-3/12 item-img-container bg-white flex items-center justify-center">
                          <img :src="item.image_url" alt="item" class="cart-dropdown-item-img p-4">
                      </div>

                      <!-- INFO COL -->
                      <div class="vx-col w-8/12 pr-4 pl-2 py-4 flex flex-col justify-center">
                          <span class="font-medium block cart-item-title truncate">{{ item.title }}</span>
                      ...
                      <!-- INFO COL -->
                      <div class="vx-col w-1/12 pr-2 pl-1 py-4 flex flex-col justify-center">
                          <feather-icon icon="XIcon" svgClasses="h-6 w-6 cursor-pointer text-danger" class="hover:text-danger" @click.stop="removeItemFromCart(item)" />
                      </div>
                  </li>
              </ul>
              </component>
              <div
                ...
                @click="$router.push('/home/cart').catch(() => {})">
